...
async function executeSQL(command, description) {
  ...
  try {
    ...
    const { stdout } = await execAsync(`npx wrangler d1 execute kimmy-app-db --remote --command="${command}"`);
...
async function setupDemoAccount() {
  ...
  try {
    ...
    const inviteCode = DEMO_CONFIG.household.inviteCode || await generateInviteCode();
    ...
    console.log(`   Role:     ${DEMO_CONFIG.user.role} (admin=${DEMO_CONFIG.user.admin})`);
