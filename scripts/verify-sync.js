...
function getTableSchema(environment, tableName) {
  ...
  try {
    ...
    console.error(`❌ Failed to get schema for ${tableName} from ${environment} database:`, error.message);
    return null;
  }
}
...
function compareSchemas() {
  ...
  const tables = ['users', 'records', 'record_types', 'quick_notes', 'contact_submissions'];
