...
function resetDatabase(environment) {
  ...
  try {
    ...
    if (
      !runCommand(dropCommand, `Dropping tables from ${environment} database`)
    ) {
    ...
    log(
      `🏗️  Recreating tables with fresh schema in ${environment} database...`,
      "info"
    );
    ...
    log(
      `🎉 Database reset completed successfully for ${environment}!`,
      "success"
    );
    log(
      `📊 Your ${environment} database now has fresh tables with the corrected timestamp schema.`,
      "success"
    );
  } catch (error) {
    log(
      `💥 Database reset failed for ${environment}: ${error.message}`,
      "error"
    );
    process.exit(1);
  }
...
function main() {
  ...
  switch (command) {
    ...
    case "remote":
      ...
      log(
        "⚠️  This will delete ALL data from your production database!",
        "warning"
      );
      log(
        "⚠️  Are you sure you want to continue? (Ctrl+C to cancel)",
        "warning"
      );
