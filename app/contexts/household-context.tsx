...
interface HouseholdContextType {
  ...
  addAdminMember: (familyId: string, userId: number, userData: { firstName: string; lastName: string; email: string }) => Promise<User>;
...
export const HouseholdProvider: React.FC<HouseholdProviderProps> = ({ children }) => {
  ...
  const addAdminMember = useCallback(async (familyId: string, userId: number, userData: { firstName: string; lastName: string; email: string }): Promise<User> => {
    try {
  ...
  const addMember = useCallback(async (familyId: string, memberData: AddMemberData): Promise<User> => {
    try {
      // Note: In the new architecture, database operations should happen in route actions
      // This is just for local state management
      const newMember: User = {
        ...
        familyId,
        role: 'member',
        age: memberData.dateOfBirth ? calculateAge(memberData.dateOfBirth) : null,
  ...
  const updateMember = useCallback(async (memberId: number, updates: Partial<User>): Promise<void> => {
    // TODO: In the new architecture, this should be handled by route actions
    setFamilyMembers(prev => 
  ...
  const getMembersByRole = useCallback((familyId: string, role: 'admin' | 'member'): User[] => {
    return familyMembers.filter(m => m.familyId === familyId && m.role === role);
  }, [familyMembers]);
