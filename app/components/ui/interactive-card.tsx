...
const INTERACTIVE_CARD_STYLES = {
  ...
  dashed: "cursor-pointer border-dashed border-2 border-slate-600 transition-all hover:border-blue-500/50 hover:shadow-xl hover:shadow-blue-500/25 bg-slate-800/50 hover:scale-[1.02] sm:hover:scale-105",
...
>(({ className, variant = "default", onClick, children, ...props }, ref) => (
  <Card
    ref={ref}
    className={cn(
      variant === "dashed" ? INTERACTIVE_CARD_STYLES.dashed : INTERACTIVE_CARD_STYLES.base,
      className
    )}
    onClick={onClick}
    {...props}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <CardTitle className="text-lg sm:text-xl text-slate-100 truncate">{title}</CardTitle>
            {description && (
              <CardDescription className="text-slate-400 text-sm line-clamp-2">{description}</CardDescription>
            )}
          </div>
        </div>
        {showChevron && (
  ...
  >
    <CardHeader className={INTERACTIVE_CARD_STYLES.header}>
      <div className="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-3 text-slate-400">
        <div className="text-2xl sm:text-3xl bg-gradient-to-r from-blue-500 to-purple-600 bg-clip-text text-transparent">➕</div>
        <div className="text-center sm:text-left">
          <CardTitle className="text-lg sm:text-xl text-slate-300">{title}</CardTitle>
          {description && (
