...
export interface UnifiedInputProps extends BaseFormFieldProps, Omit<React.InputHTMLAttributes<HTMLInputElement>, 'size' | 'onChange'> {
...
export const UnifiedInput = React.forwardRef<HTMLInputElement, UnifiedInputProps>(
  ({
    ...
    const fieldState = displayError ? 'error' : warning ? 'warning' : (value && isValid) ? 'success' : 'default';
      ...
      <div className={cn("space-y-2", className)}>
        ...
        {validation?.maxLength && type === 'text' && (
          <div className="text-xs text-right">
            <span className={cn(
              "transition-colors",
              value.length > validation.maxLength ? "text-red-400" :
              value.length > validation.maxLength * 0.8 ? "text-yellow-400" :
              "text-slate-400"
            )}>
...
export interface UnifiedTextareaProps extends BaseFormFieldProps, Omit<React.TextareaHTMLAttributes<HTMLTextAreaElement>, 'size' | 'onChange'> {
  onChange?: (value: string) => void;
  onValidationChange?: (isValid: boolean, error?: string) => void;
  resizable?: boolean;
}
...
export const UnifiedTextarea = React.forwardRef<HTMLTextAreaElement, UnifiedTextareaProps>(
  ({
    ...
    const inputId = props.id || `textarea-${React.useId()}`;
