...
export const userDb = {
  async create(env: any, userData: {
    ...
    const hashedPassword = userData.password ? 
      `hashed_${userData.password}` : null;
...
export const familyDb = {
  ...
  async addMember(env: any, memberData: {
    name: string;
    email?: string;
    familyId: string;
    role?: string;
    ...
    return await userDb.create(env, {
      ...
      email: memberData.email || `${memberData.name.toLowerCase().replace(/\s+/g, '.')}@noemail.local`,
...
export const recordTypeDb = {
  async create(env: any, recordTypeData: {
    name: string;
    description?: string;
    familyId: string;
    icon?: string;
    ...
    try {
      const result = await db.insert(schema.recordTypes).values(newRecordType).returning();
      return result[0];
    } catch (error) {
...
export const recordDb = {
  async create(env: any, recordData: {
    title: string;
    content?: string;
    recordTypeId: number;
    familyId: string;
    ...
    try {
      const result = await db.insert(schema.records).values(newRecord).returning();
      return result[0];
    } catch (error) {
...
export const authDb = {
  ...
  async createUserWithFamily(env: any, userData: {
    name: string;
    email: string;
    password: string;
    familyName?: string;
