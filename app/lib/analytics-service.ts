...
import { users, records, recordTypes, trackers, trackerEntries } from "~/db/schema";
...
export class AnalyticsService {
  constructor(private db: any, private householdId: string) {}

  async generateBasicInsights(): Promise<BasicInsights> {
    console.log("AnalyticsService.generateBasicInsights called for household:", this.householdId);
    ...
    try {
      ...
      const categoryInsights = await this.generateCategoryInsights(allRecords, allRecordTypes);

      // Generate member insights
      const memberInsights = await this.generateMemberInsights(members, allRecords);

      // Detect patterns
      const patterns = this.detectPatterns(allRecords, members, categoryInsights, memberInsights);

      // Generate recommendations
      const recommendations = this.generateRecommendations(patterns, summary, categoryInsights, memberInsights);
  ...
  private async generateSummary(members: any[], allRecords: any[]): Promise<InsightSummary> {
    const now = new Date();
    ...
    const recordsThisMonth = allRecords.filter(r => 
      r.createdAt && new Date(r.createdAt) >= oneMonthAgo
    ).length;
    ...
    const mostActiveCategory = Object.keys(categoryCounts).length > 0
    ...
    const mostActiveMemberId = Object.keys(memberCounts).length > 0
      ? Object.keys(memberCounts).reduce((a, b) => memberCounts[a] > memberCounts[b] ? a : b)
      : null;
  ...
  private async generateCategoryInsights(allRecords: any[], allRecordTypes: any[]): Promise<CategoryInsight[]> {
    ...
    for (const [category, categoryRecords] of Object.entries<any>(categoryGroups)) {
      ...
      const recentCount = records.filter(r => 
        r.createdAt && new Date(r.createdAt) >= twoWeeksAgo
      ).length;

      const previousCount = records.filter(r => 
        r.createdAt && new Date(r.createdAt) >= fourWeeksAgo && new Date(r.createdAt) < twoWeeksAgo
      ).length;
      ...
      const timeSpanWeeks = oldestRecord?.createdAt 
        ? Math.max(1, (now.getTime() - new Date(oldestRecord.createdAt).getTime()) / (7 * 24 * 60 * 60 * 1000))
        : 1;
      const averagePerWeek = Math.round((records.length / timeSpanWeeks) * 10) / 10;
  ...
  private async generateMemberInsights(members: any[], allRecords: any[]): Promise<MemberInsight[]> {
    const insights: MemberInsight[] = [];

    for (const member of members) {
      const memberRecords = allRecords.filter(r => r.memberId === member.id);
      const categories = [...new Set(memberRecords.map(r => r.category).filter(Boolean))];
      ...
      const recentCount = memberRecords.filter(r => 
        r.createdAt && new Date(r.createdAt) >= twoWeeksAgo
      ).length;

      const previousCount = memberRecords.filter(r => 
        r.createdAt && new Date(r.createdAt) >= fourWeeksAgo && new Date(r.createdAt) < twoWeeksAgo
      ).length;
  ...
  private detectPatterns(
    ...
    const healthCategories = categoryInsights.filter(c => 
    ...
    if (healthCategories.length > 0) {
      const recentHealthRecords = allRecords.filter(r => 
        healthCategories.some(hc => hc.category === r.category) &&
        r.createdAt && new Date(r.createdAt) >= new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
      );
      ...
      if (recentHealthRecords.length === 0) {
        patterns.push({
          ...
          description: "No health-related records have been logged in the past week.",
          confidence: "medium"
        });
      }
    }
    ...
    const growingCategories = categoryInsights.filter(c => c.trend === "increasing");
    ...
    const averageDaily = dailyRecords.reduce((a, b) => a + b, 0) / dailyRecords.length;
    const variance = dailyRecords.reduce((acc, val) => acc + Math.pow(val - averageDaily, 2), 0) / dailyRecords.length;

    if (variance > averageDaily * 2) { // High variance
      patterns.push({
        type: "data_entry",
  ...
  private generateRecommendations(
    ...
    const recommendations: Omit<NewAiRecommendation, "createdAt" | "updatedAt">[] = [];

    // Recommendation based on health gap pattern
    const healthGapPattern = patterns.find(p => p.type === "health" && p.title.includes("Health Data Gap"));
    if (healthGapPattern) {
      recommendations.push({
        householdId: this.householdId,
        type: "health",
        title: "Log Health Information",
        description: "Consider logging recent health information to maintain comprehensive records.",
        priority: "medium",
        status: "active"
      });
    }
    ...
    const inactivePattern = patterns.find(p => p.type === "activity" && p.title.includes("Inactive Members"));
    if (inactivePattern && inactivePattern.metadata?.inactiveMembers) {
      recommendations.push({
        ...
        priority: "low",
        status: "active",
        metadata: JSON.stringify({ memberNames: inactivePattern.metadata.inactiveMembers })
      });
    }

    // Recommendation for data consistency
    const consistencyPattern = patterns.find(p => p.type === "data_entry" && p.title.includes("Inconsistent"));
    if (consistencyPattern) {
      recommendations.push({
        householdId: this.householdId,
        type: "data_entry",
        title: "Establish Regular Data Entry",
        description: "Try to maintain a consistent schedule for logging household records.",
        priority: "low",
        status: "active"
      });
    }
    ...
    if (summary.activeCategories.length < 3) {
      recommendations.push({
        ...
        description: "Consider adding records in new categories like Health, Activities, or Development milestones.",
        priority: "medium",
        status: "active"
      });
    }
    ...
    if (summary.recordsThisWeek < 2) {
      recommendations.push({
        ...
        description: "Your household has had minimal record activity this week. Regular logging helps track important information.",
        priority: "high",
        status: "active"
      });
    }
