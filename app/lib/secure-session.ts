...
export async function createSecureToken(
  data: Omit<SecureSessionData, "iat" | "exp">,
...
export async function verifySecureToken(
  ...
  try {
    ...
    if (!headerBase64 || !payloadBase64 || !signatureBase64) {
      logger.securityEvent("Invalid token format", "medium", {
    ...
    const payloadJson = atob(
      payloadBase64.replace(/[-_]/g, char => (char === "-" ? "+" : "/"))
    );
    ...
    logger.securityEvent("Token verification error", "medium", {
 ...
 */
export async function extractSecureSession(
  request: Request,
  secret: string
): Promise<SecureSessionData | null> {
