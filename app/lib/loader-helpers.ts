...
export async function loadFamilyData(request: Request, env: any): Promise<FamilyData> {
  try {
...
export async function loadUserSession(request: Request, env: any): Promise<any> {
  try {
    // Extract session from cookies using utility function
    const session = extractSessionFromCookies(request.headers.get('cookie'));
...
export async function loadFamilyDataWithMember(
  ...
  if (familyId && memberId) {
    currentMember = familyMembers.find(member => member.id.toString() === memberId) || null;
  }
...
export async function loadFamilyDataWithCategories(
  ...
  const defaultCategories = ['Health', 'Activities', 'Personal', 'Education', 'Finance'];
