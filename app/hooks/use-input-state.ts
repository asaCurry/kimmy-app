...
export function useInputState(options: UseInputStateOptions = {}) {
  ...
  const validate = React.useCallback((value: string): { isValid: boolean; error?: string; warning?: string } => {
    ...
    // Number validations
    if ((validation.min !== undefined || validation.max !== undefined) && value) {
  ...
  const handleChange = React.useCallback((value: string) => {
    const validationResult = validateOnChange ? validate(value) : { isValid: true };
  ...
  const handleBlur = React.useCallback(() => {
    const validationResult = validateOnBlur ? validate(state.value) : { isValid: state.isValid };
